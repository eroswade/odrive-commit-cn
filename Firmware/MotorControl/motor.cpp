...
float Motor::max_available_torque() 
{
    if (config_.motor_type == Motor::MOTOR_TYPE_ACIM) 
    {
        ...
        // effective_current_lim_ = current_lim = 15自己的设置
...
// 电机控制更新程序
// 核心修改 Idq_setpoint_ 和 Vdq_setpoint_
void Motor::update(uint32_t timestamp) 
{
    // Load torque setpoint, convert to motor direction
    // @MARK  如果controller是静止的, 那么. torque为0
    // 如果用手推导致位置改变. 则torque开始出现
    ...
    float ilim = axis_->motor_.effective_current_lim_;// effective_current_lim_ = 15
    ...
    {
        // id基本上只和上一次id有关.
        id = std::clamp(id, -ilim*0.99f, ilim*0.99f); // 1% space reserved for Iq to avoid numerical issues id不起过15
    }
    ...
    if (axis_->motor_.config_.motor_type == Motor::MOTOR_TYPE_ACIM) 
    {
        ...
        // iq = torque*25
        iq = torque / axis_->motor_.config_.torque_constant; // iq=torque*25;
    }

    // 2-norm clamping where Id takes priority 
    // 限制iq< sqrt(15^2 - id^2) 也就是id+iq不大于15
    ...
    std::optional<float> phase_vel = phase_vel_src_.present();//位置角速度 不动的时候速度为 0

    if (config_.R_wL_FF_enable)//不运行 // 电感吸能
    {
        ...
        vd -= *phase_vel * config_.phase_inductance * iq;//vd -= 0
        vq += *phase_vel * config_.phase_inductance * id;// vq += 0
        vd += config_.phase_resistance * id;// vd = vd+0.082*id
        vq += config_.phase_resistance * iq;// vq = vq+0.082*iq
    }

    if (config_.bEMF_FF_enable)//不运行 // 电动势计算 计算反电动势增加的力, 要超过这个力才好运行
    {
        if (!phase_vel.has_value()) 
        {
            error_ |= ERROR_UNKNOWN_PHASE_VEL;
    ...
    {
        Vdq_setpoint_ = {vd, vq};// 上面两个都不运行的话. 这里设置为0了??
    }
}
...
/**
 * @brief Called when the underlying hardware timer triggers an update event.
 * 这个函数得到的结果,只给 current_meas_cb 检查是否超max
 */
...
void Motor::pwm_update_cb(uint32_t output_timestamp) 
{
    ...
    // Apply control law to calculate PWM duty cycles 控制PWM的duty cycles
