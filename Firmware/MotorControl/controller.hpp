...
public:
    ...
    struct Config_t {
        ...
        bool circular_setpoints = false; // 此模式对于连续增量位置移动很有用。
    ...
    float pos_setpoint_ = 0.0f; // [turns] 当前运行位置
    float vel_setpoint_ = 0.0f; // [turn/s] 当前运行速度
    // float vel_setpoint = 800.0f; <sensorless example>
    float vel_integrator_torque_ = 0.0f;    // [Nm]
    float torque_setpoint_ = 0.0f;  // [Nm]

    float input_pos_ = 0.0f;     // [turns] 输入运行目标位置
    float input_vel_ = 0.0f;     // [turn/s]
    float input_torque_ = 0.0f;  // [Nm]
    float input_filter_kp_ = 0.0f;// 4
    float input_filter_ki_ = 0.0f;// 4
