...
void Encoder::sample_now() 
{
    switch (mode_) 
    {
        ...
        case MODE_SPI_ABS_MA732: // 绝对值编码器
        {
            abs_spi_start_transaction();// 从SPI读
            // Do nothing
        } break;
...
bool Encoder::update() {
    ...
    switch (mode_) {
        case MODE_INCREMENTAL: {
            ...
            //or use 64 bit
            int16_t delta_enc_16 = (int16_t)tim_cnt_sample_ - (int16_t)shadow_count_;//tim_cnt_sample_ 由sample_now完成.
    ...
    shadow_count_ += delta_enc; // shadow_count_自增  记录所有的传感器数据累加(内部绝对值传感器?)
    count_in_cpr_ += delta_enc; // 在一圈内的count 比如一圈config_.cpr=16384 会按照这个数来求模
    count_in_cpr_ = mod(count_in_cpr_, config_.cpr); // 重新限制在每一圈内.

    if(mode_ & MODE_FLAG_ABS)//如果是绝对值编码器
        count_in_cpr_ = pos_abs_latched;

    // Memory for pos_circular pos占了多少圈
    float pos_cpr_counts_last = pos_cpr_counts_;// 先存住pos_cpr_counts_ 这个值和count_in_cpr_很像. 只是是浮点型

    //// run pll (for now pll is in units of encoder counts)
    // Predict current pos 预测当前位置
    pos_estimate_counts_ += current_meas_period * vel_estimate_counts_;// 这个值约为shadow_count_ 预测值
    ...
    // discrete phase detector 测量值-预测值
    float delta_pos_counts = (float)(shadow_count_ - encoder_model(pos_estimate_counts_));// 总数运行传感值上的差值
    float delta_pos_cpr_counts = (float)(count_in_cpr_ - encoder_model(pos_cpr_counts_));// 圈内数的浮点差
    delta_pos_cpr_counts = wrap_pm(delta_pos_cpr_counts, (float)(config_.cpr));
    delta_pos_cpr_counts_ += 0.1f * (delta_pos_cpr_counts - delta_pos_cpr_counts_); // for debug 加权平均 只能监视数据用
    // pll feedback 
    pos_estimate_counts_ += current_meas_period * pll_kp_ * delta_pos_counts;// 更新总数和圈内数的预测系数
    pos_cpr_counts_ += current_meas_period * pll_kp_ * delta_pos_cpr_counts;// 更新圈内差值量.加回到预测值
    pos_cpr_counts_ = fmodf_pos(pos_cpr_counts_, (float)(config_.cpr));
    vel_estimate_counts_ += current_meas_period * pll_ki_ * delta_pos_cpr_counts;// 更新恒速模型
    ...
    // Outputs from Encoder for Controller 最后的pos_estimate_ 和vel_estimate_
    pos_estimate_ = pos_estimate_counts_ / (float)config_.cpr; // 除掉一圈的脉冲量.就是当前位置和当前速度
    ...
    // the encoder from error states. pos_circular在圈内的位置, [0-1]
    ...
    //// run encoder count interpolation 插值
    ...
    float interpolated_enc = corrected_enc + interpolation_;// 最后的插值,根据圈内脉冲数 增加0-1之间一个值

    //// compute electrical phase 计算电角度(pole_pairs 7*2 * 3.1415 * (1/16384))
    //TODO avoid recomputing elec_rad_per_enc every time
    float elec_rad_per_enc = axis_->motor_.config_.pole_pairs * 2 * M_PI * (1.0f / (float)(config_.cpr));
    // 又减掉大约一个0.5 * 每一脉冲量的电角度 得到真正的电角度
    float ph = elec_rad_per_enc * (interpolated_enc - config_.phase_offset_float);
    
    if (is_ready_) {
        phase_ = wrap_pm_pi(ph) * config_.direction; // 电角度的真实值(带方向)
        phase_vel_ = (2*M_PI) * *vel_estimate_.present() * axis_->motor_.config_.pole_pairs * config_.direction; // 电角度速度
    }
