...
// 用信号输入的方式控制ODRIVE
// 如果是信号输入方式控制 step_dir_active_被设置为true
...
bool Axis::start_closed_loop_control() {
    ...
    // Hook up the data paths between the components 临界资源
    CRITICAL_SECTION() 
    {
        if (sensorless_mode) // 如果是无传感器
        {
...
bool Axis::run_closed_loop_control_loop() {
    ...
    // 这里延时. 代码在别的地方. 如果没猜错,应该是在motor里的update
    while ((requested_state_ == AXIS_STATE_UNDEFINED) && motor_.is_armed_) {
        osDelay(1);
    }

    set_step_dir_active(config_.enable_step_dir && config_.step_dir_always_on);
    stop_closed_loop_control(); // 停止闭环
